...
const TrialBal = () => {
    ...
    const getTrialReport = () => {
        ...
        const url = `${REACT_APP_URL}/Voucher/trialbal/${societyId}?fromDate=${formatDate(fromDate)}&toDate=${formatDate(toDate)}`;
    ...
    // ✅ TOTALS
    const totalDebit = trialBalData.reduce(
        (sum, row) => sum + (Number(row.TotalDrAmount) || 0), 0
    );

    const totalCredit = trialBalData.reduce(
        (sum, row) => sum + (Number(row.TotalCrAmount) || 0), 0
    );
    ...
    const generatePDF = () => {
        ...
        const rows = trialBalData.map((row) => {
            const closing =
                Number(row.opening || 0) +
                Number(row.TotalDrAmount || 0) -
                Number(row.TotalCrAmount || 0);
        ...
        autoTable(doc, {
            startY: 80,
            head: [["Ledger", "Opening", "Debit", "Credit", "Closing"]],
            body: rows
        });
    ...
    return (
        <Box>
            ...
            <Box textAlign="center">
                <Button variant="contained" onClick={handleGetTrialReport}>
                    Get Report
                </Button>
            </Box>
